...
public class CashReportView extends JPanel {
		...
		btnGo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showError(CashReportView.this, POSConstants.ERROR_MESSAGE, e1);
				}
			}
	...
	private void viewReport() throws Exception {
		...
		map.put("reportTitle", Messages.getString("CashReportView.0")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		JasperReport jasperReport = ReportUtil.getReport("cash-report"); //$NON-NLS-1$
		JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, map, new JRTableModelDataSource(new CashReportModel(transactions)));
	...
	public static void main(String[] args) {
		...
		PanelTester.test(new CashReportView());
	}
}
