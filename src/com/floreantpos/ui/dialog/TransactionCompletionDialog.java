...
public class TransactionCompletionDialog extends POSDialog {
	...
	private JLabel lblGratuityAmount;
        ...
        private boolean isGroupSettle = false;
	...
	private void initializeComponents() {
		...
		btnPrintStoreCopy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
//                                        POSMessageDialog.showMessage("XXXXXXX");
					ReceiptPrintService.printTransaction(completedTransaction, false);
                                        
                                        // Add code to check ticket has been print to customer
//                                        POSMessageDialog.showMessage("XXXXXXX - 2");
                                        if (isGroupSettle) {
                                            for (PosTransaction transaction: completedTransactions) {
                                                Ticket ticket = transaction.getTicket();
                                                ticket.setPrintedCustomerCopy(true);
		...
		JPanel p = new JPanel();
                ...
                // Should add code to determine print times here
//		if (isCard) {
