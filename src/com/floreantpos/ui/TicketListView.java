		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			...
			switch (columnIndex) {
				...
				case 8:
                                    ...
                                    if (ticket.getTransactions().size() > 0) {
                                        String pt = ticket.getTransactions().iterator().next().getPaymentType();
//                                        System.out.println("Payment type = " + pt);
                                        if (PaymentType.CREDIT_CARD.getDisplayString().equals(pt))
                                                paymentType = "CC";
					...
					if (ticket.isVoided()) {
						...
						status += Messages.getString("TicketListView.13") + " (" + paymentType + ")"; //$NON-NLS-1$
					}
                                        else if (ticket.isPaid()) {
						status += " (" + paymentType + ")"; //$NON-NLS-1$
					}
