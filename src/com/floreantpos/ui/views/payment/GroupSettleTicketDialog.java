...
public class GroupSettleTicketDialog extends POSDialog implements CardInputListener {
		...
		try {
			...
			switch (paymentType) {
				...
				case CREDIT_DISCOVERY:
					payUsingCard(totalTenderAmount);
					break;
    ...
    private void payUsingCard(final double tenderedAmount) throws Exception {
        CardPaymentDialog paymentDialog = new CardPaymentDialog(this);
        paymentDialog.pack();
        paymentDialog.open();
	...
	public void cardInputted(CardInputProcessor inputter, PaymentType selectedPaymentType) {
		...
		try {
			...
			if (inputter instanceof SwipeCardDialog) {
				...
				settleTicket(selectedTransaction);
			} else if (inputter instanceof CardPaymentDialog) {
                                                            ...
                                                            settleTicket(null, selectedTransaction);
                                                        }
		} catch (Exception e) {
                    e.printStackTrace();
			PosLog.error(getClass(), e);
